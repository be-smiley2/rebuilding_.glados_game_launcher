"""Steam library discovery utilities for the Aperture Science launcher."""
...
def print_game_report(games: Sequence[SteamGame]) -> None:
    ...
    if not games:
        print(f"{SYSTEM_ALERT}No Steam games were detected on this system.")
        print(
            f"{SYSTEM_ALERT}Make sure your Steam libraries are installed on local drives."
        )
        return

    print(f"{SYSTEM_SUCCESS}Found {len(games)} Steam game(s):\n")
    for index, game in enumerate(games, start=1):
        print(f"{SYSTEM_PRIMARY}{index}. {game.pretty()}\n")
